        ...
        with open(log_file, "r") as fin:
            for line in fin.readlines():
                try:
                    match = regex.search(line.strip())
                    message = [match.group(header) for header in headers]
                    if ex_content!="": #supposed no exceptions threw
                        lastlineid = linecount-1
                        log_messages[lastlineid][5] = log_messages[lastlineid][5]+ex_content
                        ex_content = ""
                    log_messages.append(message)
                    linecount += 1
                except Exception as e:
                    # pass
                    # if "at" in line or "Cause by" in line:
                    if "2023" not in line:
                        ex_content = ex_content + line
                    else:
                        print("[Warning] Skip line: " + line)
        logdf = pd.DataFrame(log_messages, columns=headers)
        logdf.insert(0, "LineId", None)
